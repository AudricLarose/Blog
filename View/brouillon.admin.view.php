 ...
 foreach ($brouillon as $brouillons) {
 	?><tr><td class="center">
 <a href="index.php?action=modification&id=<?= $brouillons['id'] ?>"><p><?= $brouillons['title'] ?></p></a>
<form action='index.php?action=montrer_ecriture' method='POST'>
<input name="idk" type="hidden" value="<?= $brouillons['id']?>"/>
<button name='supprimer_brouillon' class ='suppression' value='supprimer'>Supprimer</button></form></td></tr>
<?php }  ?>
</table>
</div>
